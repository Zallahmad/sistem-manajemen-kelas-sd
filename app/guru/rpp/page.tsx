...
export default async function GuruLessonPlanPage({
  ...
  const totalPublished = plansList.filter((m) => m.status === "PUBLISHED").length;
  const totalDraft = plansList.filter((m) => m.status === "DRAFT").length;
  const totalReview = plansList.filter((m) => m.status === "REVIEW").length;

  return (
    <AppLayout user={user}>
      <PageHeader
        title="Rencana Pelaksanaan Pembelajaran (RPP)"
        subtitle="Kelola dokumen rencana kegiatan tatap muka per kelas dan mata pelajaran"
        action={
          <Link
            href="/guru/rpp/new"
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            icon="🚀"
            description="Siap digunakan mengajar"
          />
          <StatCard
          ...
          <form method="GET" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-3">
            <div className="lg:col-span-2">
              ...
              <input
                ...
                name="search"
                defaultValue={search || ""}
                placeholder="Cari kata kunci RPP..."
          ...
          {plansList.length === 0 ? (
            <div className="py-12 text-center text-xs text-zinc-500 space-y-2">
              <p className="font-semibold text-zinc-700 dark:text-zinc-300 text-sm">
                Belum ada Rencana Pelaksanaan Pembelajaran (RPP) ditemukan.
              </p>
              <p>Mulai susun RPP baru untuk kegiatan belajar mengajar Anda.</p>
              <div className="pt-2">
                <Link
                  href="/guru/rpp/new"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left">
                <thead className="border-b border-zinc-200 dark:border-zinc-800 text-zinc-500">
                  <tr>
                    <th className="py-2.5 px-3">Judul RPP & Topik</th>
